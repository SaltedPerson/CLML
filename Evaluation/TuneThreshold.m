...
% data with one evaluation metric
    if nargin < 3 % nargin为内置函数表示输入参数的个数
        bAllOne = 1;
    elseif nargin < 4
        metricIndex = 3; % F1 or label-based F1
    end
    %fprintf('-- Tune threshold for multi-label classification\n');
    [num_class,num_train] = size(target);%此时的target是转置的
    TotalNums = 50;
    %min_score = min(min(output));
    min_score = 0;
    max_score = max(max(output));
    step = (max_score - min_score)/TotalNums;
    tau_range = min_score:step:max_score;%tau_range类似于一个元组，从min--max,每个数的间距为step
    
    tau = zeros(1,num_class);%用来存放每个类标签的阈值，初始[0,0,0,0,0,0,0...]
    currentResult = tau;
    for t = 1:length(tau_range)
        ...
        if bAllOne == 1 % set to only one threshold for all the class labels
            ...
            predict_target = single( (output - thresholds) >= 0 );%满足条件置为1，反之为0
            tempResult = evaluateOneMetric(target, predict_target, metricIndex);%得到某一评价指标的值
            if tempResult > currentResult(1,1)
                %记录最好的结果以及  对应的阈值
                currentResult(1,1) = tempResult;
                tau(1,1) = threshold;
            end
        else % set to one threshold for each label independently 为每个标签都寻找一个最合适的阈值
            for c = 1:num_class
